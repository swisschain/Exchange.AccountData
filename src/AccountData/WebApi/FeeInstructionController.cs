...
namespace AccountData.WebApi
{
    ...
    public class FeeInstructionController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetManyAsync([FromQuery] FeeInstructionRequestMany request)
        {
            ...
            var domains = await _feeInstructionService.GetAllAsync(brokerId, request.Id, request.FromWalletId, request.ToWalletId, request.OrderId, request.AssetId, sortOrder, request.Cursor, request.Limit);
